...
export function SignUp() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage('이메일로 가입 링크를 보냈습니다. 메일함을 확인하세요.');
    } catch (error) {
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-semibold tracking-tight text-slate-900">회원가입</h1>
      <p className="mt-2 text-sm leading-relaxed text-slate-600">이메일 매직 링크로 계정을 만드세요.</p>
      ...
      <p className="mt-6 border-t border-slate-200 pt-5 text-sm text-slate-600">
        이미 계정이 있나요?{' '}
        <Link to="/signin" className="font-semibold text-slate-900 underline decoration-slate-300 underline-offset-4">
          로그인
        </Link>
      </p>
    </motion.main>
